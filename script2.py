                ...
                for label in labels:
                    if label.text.strip() == end_time:
                        ...
                        try:
                            ...
                            if not captcha_ok:
                                print("验证码失败，重试本次预约")
                                raise  # 跳出end_time循环，进入下一次attempt
                        except TimeoutException:
                            print("无验证码弹窗，正常继续")
...
def main():
    ...
    sit_avilable, day_type = None, None
    users = {"自定义": [account_password4, "三楼理科书库", "88"]}
    user = "自定义"
...
if __name__ == "__main__":
    ...
    for _retry in range(MAX_SCRIPT_RETRIES):
        try:
            main()
            break  # 成功就退出
        except SystemExit:
            ...
            print(f"脚本第{_retry+1}次运行失败: {e}")
            import traceback
            traceback.print_exc()
            if _retry < MAX_SCRIPT_RETRIES - 1:
                print(f"5秒后重启脚本（第{_retry+2}次）...")
                time.sleep(5)
            else:
                print("已达最大重试次数，退出")